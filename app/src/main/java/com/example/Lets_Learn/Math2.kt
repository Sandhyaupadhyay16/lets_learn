...
class Math2 : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val math2play = findViewById<Button>(R.id.math2playbtn)
        val math2stop = findViewById<Button>(R.id.math2stopbtn)
        ...
        math2play.setOnClickListener {
            ...
            try {
                if (!mp!!.isPlaying) {
                    mp?.setDataSource(
                        this,
                        Uri.parse("android.resource://" + this.packageName + "/" + R.raw.two)
                    )
                    mp?.prepare()
                    mp?.start()
                }
